...
# currently activating the LSA package seems to crash the script on github
# now trying a direct call to the Cosine function with the package unactivated

# install.packages("LSAfun")

packagelist <- c('bookdown','knitr','rmarkdown','osfr','plyr','tidyr','dplyr','emmeans','ggplot2','Rmisc','buildmer','lme4','lmerTest','stringr','effects','effectsize','ggdist','car','gridExtra','kableExtra','rstatix','simr','effsize','readr')
...
for (i in 1:length(intrusion_item)){
  new_element <- LSAfun::Cosine(lure_item[1], intrusion_item[i], ukwac)
  my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[2], intrusion_item[i], ukwac)
  my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[3], intrusion_item[i], ukwac)
  my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[4], intrusion_item[i], ukwac)
  my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[5], intrusion_item[i], ukwac)
  my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[6], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[7], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[8], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[9], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[10], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[11], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[12], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[13], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[14], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[15], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[16], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[17], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[18], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[19], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  new_element <- LSAfun::Cosine(lure_item[20], intrusion_item[i], ukwac)
   my_list[[length(my_list) + 1]] <- new_element
  }
